...
public class Signup extends AppCompatActivity {
    ...
    private void saveUserData(String name, String email, String userId, String photoUrl) {
        ...
        userMap.put("uid", uid); // Add Firebase UID
        userMap.put("name", name);
        userMap.put("email", email);
        userMap.put("photo", (photoUrl != null) ? photoUrl : "");
        userMap.put("online", "true"); // Add online status

        // Save the user data under the "Users" node with the userId as the key
        userDatabase.child(userId).setValue(userMap)
                .addOnCompleteListener(dbTask -> {
                    if (dbTask.isSuccessful()) {
                        saveUserToken(userId); // Save the device token
                    } else {
